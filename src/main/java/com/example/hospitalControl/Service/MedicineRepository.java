...
public interface MedicineRepository extends JpaRepository<Medicine, Integer> {
	...
	@Query("SELECT m FROM Medicine m "
			...
			+ "AND ((m.price >= :price - 50000 AND m.price <= :price + 50000) OR :price = 0)"
			+ "AND m.quantity > 0")
	Page<Medicine> findByNameExpirationDateQuantityPrice1(String name, LocalDate expirationDate, int quantity, int price,
			Pageable pageable);
	
	@Query("SELECT m FROM Medicine m " +
			"WHERE m.quantity > 0")
	Page<Medicine> findByQuantity(Pageable pageable);
}
